...
case "long-short": {
  ...
  try {
    ...
    const coins = response.data.data;
    const btc = coins.find(c => String(c.symbol || "").toUpperCase() === "BTC");

    if (!btc || !Number.isFinite(Number(btc.long_short_ratio_24h)) || Number(btc.long_short_ratio_24h) <= 0) {
      return res.json({ long_pct: "0.00", short_pct: "0.00", differential: "0.00", method: "no-valid-ratios" });
    }

    const avgRatio = Number(btc.long_short_ratio_24h);
    ...
    const payload = {
      ...
      sampled_coins: [{
        symbol: btc.symbol,
        market_cap_usd: btc.market_cap_usd,
        open_interest_usd: btc.open_interest_usd,
        long_short_ratio_24h: btc.long_short_ratio_24h
      }],
      lastUpdated: new Date().toISOString(),
      method: "live-fetch-btc-24h"
    };
...
case "market-cycle-roi": {
  ...
  // 3) Live calculation with strict cycle boundaries
  try {
    console.log(`DEBUG [${type}]: Calculating Bitcoin Market Cycle ROI...`);
    
    // Define market cycle bottoms with STRICT END DATES
    const marketCycles = [
      {
        name: "Cycle 1 (2011-2013)",
        bottomDate: "2011-11-18",
        endDate: "2015-01-13", // Day BEFORE Cycle 2 starts
        bottomPrice: 2.01,
        color: "#ff4e00"
      },
      {
        ...
        endDate: "2018-12-14", // Day BEFORE Cycle 3 starts
        bottomPrice: 152,
        color: "#2ed573"
      },
      {
        ...
        endDate: "2022-11-20", // Day BEFORE Cycle 4 starts
        bottomPrice: 3126,
        color: "#ffa502"
      },
      {
    ...
    // Fetch Bitcoin historical data directly from Google Sheets API
    const btcHistoryResponse = await axiosWithBackoff(() =>
      axios.get(`https://${req.headers.host || 'whale-trades.vercel.app'}/api/data?type=bitcoin-historical`, {
        timeout: 15000,
        validateStatus: s => s < 500
      })
    );
    
    if (!btcHistoryResponse.data?.success || !Array.isArray(btcHistoryResponse.data?.data)) {
      throw new Error("Failed to fetch Bitcoin historical data");
    }

    const allHistoricalData = btcHistoryResponse.data.data;
    console.log(`DEBUG [${type}]: Loaded ${allHistoricalData.length} total historical data points`);

    // Process each cycle with STRICT date filtering
    const cycleData = marketCycles.map(cycle => {
      const bottomDate = new Date(cycle.bottomDate);
      const endDate = cycle.endDate ? new Date(cycle.endDate) : new Date(); // Use today for current cycle
      const roiData = [];

      console.log(`DEBUG [${type}]: Processing ${cycle.name} from ${cycle.bottomDate} to ${cycle.endDate || 'current'}`);

      // Filter data points within STRICT cycle boundaries
      const cycleData = allHistoricalData.filter(point => {
        // Handle different possible date field names
        const dateStr = point.date || point.DATE || point.Date;
        if (!dateStr) {
          console.log(`DEBUG: Skipping point with no date field:`, Object.keys(point));
          return false;
        }
        
        const pointDate = new Date(dateStr);
        if (isNaN(pointDate.getTime())) {
          console.log(`DEBUG: Skipping point with invalid date: ${dateStr}`);
          return false;
        }
        
        // STRICT: Must be >= bottom date AND < end date
        const afterBottom = pointDate >= bottomDate;
        const beforeEnd = cycle.endDate ? pointDate <= endDate : true; // No end restriction for current cycle
        
        const isInCycle = afterBottom && beforeEnd;
        
        if (!isInCycle && pointDate >= bottomDate && pointDate > endDate) {
          // This helps debug data bleeding
          console.log(`DEBUG: Excluding ${dateStr} from ${cycle.name} (after cycle end ${cycle.endDate})`);
        }
        
        return isInCycle;
      });

      console.log(`DEBUG [${type}]: ${cycle.name} - Found ${cycleData.length} data points within strict cycle boundaries`);

      // Calculate ROI for each valid data point
      cycleData.forEach(point => {
        const dateStr = point.date || point.DATE || point.Date;
        const pointDate = new Date(dateStr);
        const daysSinceBottom = Math.floor((pointDate - bottomDate) / (1000 * 60 * 60 * 24));
        
        // Get price - handle different field names and quote removal
        let price = point.close || point.PRICE || point.price || point.Close;
        ...
        if (!isNaN(price) && price > 0 && daysSinceBottom >= 0) {
          ...
          roiData.push({
            daysSinceBottom,
            roi: Math.max(roi, -99), // Cap negative ROI
            price,
            date: dateStr
          });
        }
      ...
      console.log(`DEBUG [${type}]: ${cycle.name} - Generated ${roiData.length} ROI data points`);
      if (roiData.length > 0) {
        console.log(`DEBUG [${type}]: ${cycle.name} - Date range: ${roiData[0].date} to ${roiData[roiData.length - 1].date}`);
        console.log(`DEBUG [${type}]: ${cycle.name} - Price range: $${roiData[0].price} to $${roiData[roiData.length - 1].price}`);
      }
    ...
    const chartSeries = validCycles.map(cycle => ({
      name: cycle.name,
      type: 'line',
      data: cycle.data.map(point => [point.daysSinceBottom, Math.max(point.roi, 0.01)]),
...
case "bitcoin-daily": {
  const TTL = 3600; // Cache for 1 hour
  const cacheKey = "bitcoin-daily-data";
  const lastGoodKey = "last:bitcoin-daily-data";

  // 1) Fast lane - check cache first
  const cached = await kv.get(cacheKey);
  if (cached) {
    console.log(`DEBUG [${type}]: Returning cached Bitcoin daily data`);
    return res.json(cached);
  }

  // 2) Traffic cop
  if (!(await allow("sheets:bitcoin-daily", 50))) {
    console.log(`DEBUG [${type}]: Rate limit active. Serving last known good`);
    const last = await kv.get(lastGoodKey);
    if (last) return res.json(last);
    return res.json({
  ...
  // 3) Live fetch from Google Sheets
  try {
    console.log(`DEBUG [${type}]: Fetching Bitcoin daily data from Google Sheets`);
    
    // Use the published CSV URL from Google Sheets for daily data
    const publishedCsvUrl = "https://docs.google.com/spreadsheets/d/e/2PACX-1vSzIZzdLBGrxBIDyI9Izd7YmW4bMLcMnHCC6kX5qYaDG-F9WJXyncRziBp4Jl7da6U2MCDB35MNJzfb/pub?output=csv";
    const csvUrls = [
      publishedCsvUrl,
      // Fallback URLs in case the published URL changes
      "https://docs.google.com/spreadsheets/d/e/2PACX-1vSzIZzdLBGrxBIDyI9Izd7YmW4bMLcMnHCC6kX5qYaDG-F9WJXyncRziBp4Jl7da6U2MCDB35MNJzfb/pub?output=csv&gid=0"
    ];
    ...
    return res.status(500).json({
      success: false,
      data: [],
      error: "Failed to fetch Bitcoin daily data",
      message: err.message,
  ...
  try {
    ...
    for (let i = 1; i < lines.length; i++) {
      ...
      if (values.length >= 5) {
        ...
        // Validate that prices make sense (basic sanity check for gold prices)
        if (open < 1 || high < 1 || low < 1 || close < 1 || close > 10000) {
          console.log(`DEBUG [${type}]: Skipping row ${i} with invalid gold prices: O:${open} H:${high} L:${low} C:${close}`);
          continue;
        }
...
case "nasdaq-dca": {
  const TTL = 3600; // Cache for 1 hour
  const { frequency = "daily" } = req.query; // daily or weekly
  const cacheKey = `nasdaq-dca-data-${frequency}`;
  const lastGoodKey = `last:nasdaq-dca-data-${frequency}`;

  // 1) Fast lane - check cache first
  const cached = await kv.get(cacheKey);
  if (cached) {
    console.log(`DEBUG [${type}]: Returning cached Nasdaq ${frequency} data`);
    return res.json(cached);
  }

  // 2) Traffic cop
  if (!(await allow("sheets:nasdaq", 50))) {
    console.log(`DEBUG [${type}]: Rate limit active. Serving last known good`);
    const last = await kv.get(lastGoodKey);
    if (last) return res.json(last);
    return res.json({
  ...
  // 3) Live fetch from Google Sheets
  try {
    console.log(`DEBUG [${type}]: Fetching Nasdaq ${frequency} data from Google Sheets`);
    
    // Nasdaq historical data CSV URL
    const publishedCsvUrl = "https://docs.google.com/spreadsheets/d/e/2PACX-1vSsIQn21xt1r7s5l2YRRXy0YejK_ZYDZLh0Lpbk1OFVmmZuvDFXF6084jJQcE79I8PBv2OkWaf7_iIp/pub?output=csv";
    const csvUrls = [
      publishedCsvUrl,
      // Fallback URLs if needed
      `${publishedCsvUrl}&gid=0`
    ...
    console.log(`DEBUG [${type}]: Processing ${lines.length} lines from Nasdaq CSV`);
    ...
    for (let i = 1; i < lines.length; i++) {
      ...
      if (values.length >= 5) {
        ...
        // Validate that prices make sense (basic sanity check for Nasdaq prices)
        if (open < 10 || high < 10 || low < 10 || close < 10 || close > 100000) {
          console.log(`DEBUG [${type}]: Skipping row ${i} with invalid Nasdaq prices: O:${open} H:${high} L:${low} C:${close}`);
          continue;
        }
    ...
    if (dailyData.length === 0) {
      throw new Error("No valid Nasdaq price data found in the sheet");
    }
    ...
    console.log(`DEBUG [${type}]: Successfully processed ${finalData.length} ${frequency} Nasdaq records`);
    console.log(`DEBUG [${type}]: Date range: ${payload.date_range.start} to ${payload.date_range.end}`);
    console.log(`DEBUG [${type}]: Current price: $${payload.current_price}`);
    ...
    return res.status(500).json({
      success: false,
      data: [],
      error: "Failed to fetch Nasdaq DCA data",
      message: err.message,
      method: "error-fallback"
    });
  }
